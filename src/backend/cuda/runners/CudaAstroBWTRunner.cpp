...
constexpr uint32_t evildragon::CudaAstroBWTRunner::BWT_DATA_STRIDE;


evildragon::CudaAstroBWTRunner::CudaAstroBWTRunner(size_t index, const CudaLaunchData &data) :
...
bool evildragon::CudaAstroBWTRunner::run(uint32_t startNonce, uint32_t *rescount, uint32_t *resnonce)
{
    return callWrapper(CudaLib::astroBWTHash(m_ctx, startNonce, m_target, rescount, resnonce));
}


bool evildragon::CudaAstroBWTRunner::set(const Job &job, uint8_t *blob)
{
...
size_t evildragon::CudaAstroBWTRunner::roundSize() const
{
...
size_t evildragon::CudaAstroBWTRunner::processedHashes() const
{
