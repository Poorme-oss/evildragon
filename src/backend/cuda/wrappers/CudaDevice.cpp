...
evildragon::CudaDevice::CudaDevice(uint32_t index, int32_t bfactor, int32_t bsleep) :
    m_index(index)
{
...
evildragon::CudaDevice::CudaDevice(CudaDevice &&other) noexcept :
...
evildragon::CudaDevice::~CudaDevice()
{
    CudaLib::release(m_ctx);
}


size_t evildragon::CudaDevice::freeMemSize() const
{
    return CudaLib::deviceUlong(m_ctx, CudaLib::DeviceMemoryFree);
}


size_t evildragon::CudaDevice::globalMemSize() const
{
    return CudaLib::deviceUlong(m_ctx, CudaLib::DeviceMemoryTotal);
}


uint32_t evildragon::CudaDevice::clock() const
{
    return CudaLib::deviceUint(m_ctx, CudaLib::DeviceClockRate) / 1000;
}


uint32_t evildragon::CudaDevice::computeCapability(bool major) const
{
    return CudaLib::deviceUint(m_ctx, major ? CudaLib::DeviceArchMajor : CudaLib::DeviceArchMinor);
}


uint32_t evildragon::CudaDevice::memoryClock() const
{
    return CudaLib::deviceUint(m_ctx, CudaLib::DeviceMemoryClockRate) / 1000;
}


uint32_t evildragon::CudaDevice::smx() const
{
    return CudaLib::deviceUint(m_ctx, CudaLib::DeviceSmx);
}


void evildragon::CudaDevice::generate(const Algorithm &algorithm, CudaThreads &threads) const
{
...
void evildragon::CudaDevice::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
