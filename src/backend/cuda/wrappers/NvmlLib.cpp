...
bool evildragon::NvmlLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_loader      = fileName;
        m_ready       = dlopen() && load();
...
const char *evildragon::NvmlLib::lastError() noexcept
{
    return uv_dlerror(&nvmlLib);
}


void evildragon::NvmlLib::close()
{
    if (m_ready) {
        pNvmlShutdown();
    }
...
bool evildragon::NvmlLib::assign(std::vector<CudaDevice> &devices)
{
    uint32_t count = 0;
...
NvmlHealth evildragon::NvmlLib::health(nvmlDevice_t device)
{
    if (!device) {
        return {};
    }
...
bool evildragon::NvmlLib::dlopen()
{
    if (!m_loader.isNull()) {
