...
evildragon::CudaThread::CudaThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
evildragon::CudaThread::CudaThread(uint32_t index, nvid_ctx *ctx) :
...
bool evildragon::CudaThread::isEqual(const CudaThread &other) const
{
...
rapidjson::Value evildragon::CudaThread::toJSON(rapidjson::Document &doc) const
{
