...
#ifdef evildragon_FEATURE_NVML
#include "backend/cuda/wrappers/NvmlLib.h"

namespace evildragon { static const char *kNvmlLabel = "NVML"; }
#endif
...
const char *evildragon::cuda_tag()
{
    return Tags::nvidia();
}


evildragon::CudaBackend::CudaBackend(Controller *controller) :
...
evildragon::CudaBackend::~CudaBackend()
{
    delete d_ptr;
...
bool evildragon::CudaBackend::isEnabled() const
{
    return d_ptr->controller->config()->cuda().isEnabled() && CudaLib::isInitialized() && !d_ptr->devices.empty();;
}


bool evildragon::CudaBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cuda().threads().get(algorithm).isEmpty();
}


const evildragon::Hashrate *evildragon::CudaBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const evildragon::String &evildragon::CudaBackend::profileName() const
{
    return d_ptr->profileName;
}


const evildragon::String &evildragon::CudaBackend::type() const
{
    return kType;
}


void evildragon::CudaBackend::execCommand(char)
{
}


void evildragon::CudaBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void evildragon::CudaBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void evildragon::CudaBackend::printHealth()
{
#   ifdef evildragon_FEATURE_NVML
    d_ptr->printHealth();
#   endif
...
void evildragon::CudaBackend::setJob(const Job &job)
{
...
void evildragon::CudaBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void evildragon::CudaBackend::stop()
{
...
void evildragon::CudaBackend::tick(uint64_t ticks)
{
...
#ifdef evildragon_FEATURE_API
rapidjson::Value evildragon::CudaBackend::toJSON(rapidjson::Document &doc) const
{
...
void evildragon::CudaBackend::handleRequest(IApiRequest &)
{
}
#endif
