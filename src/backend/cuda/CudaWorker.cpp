...
evildragon::CudaWorker::CudaWorker(size_t id, const CudaLaunchData &data) :
...
evildragon::CudaWorker::~CudaWorker()
{
    delete m_runner;
}


void evildragon::CudaWorker::jobEarlyNotification(const Job& job)
{
    if (m_runner) {
...
bool evildragon::CudaWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t evildragon::CudaWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void evildragon::CudaWorker::start()
{
...
bool evildragon::CudaWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::CUDA) == 0) {
        return false;
    }
...
void evildragon::CudaWorker::storeStats()
{
    if (!isReady()) {
        return;
    }
