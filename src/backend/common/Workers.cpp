...
{
public:
    evildragon_DISABLE_COPY_MOVE(WorkersPrivate)
...
template<class T>
const evildragon::Hashrate *evildragon::Workers<T>::hashrate() const
{
...
template<class T>
void evildragon::Workers<T>::setBackend(IBackend *backend)
{
...
template<class T>
void evildragon::Workers<T>::start(const std::vector<T> &data)
{
...
template<class T>
void evildragon::Workers<T>::stop()
{
...
template<class T>
void evildragon::Workers<T>::tick(uint64_t)
{
    if (!d_ptr->hashrate) {
        return;
    }
...
template<class T>
evildragon::IWorker *evildragon::Workers<T>::create(Thread<T> *)
{
    return nullptr;
}
...
template<class T>
void evildragon::Workers<T>::onReady(void *arg)
{
...
template<>
evildragon::IWorker *evildragon::Workers<CpuLaunchData>::create(Thread<CpuLaunchData> *handle)
{
...
#ifdef evildragon_FEATURE_OPENCL
template<>
evildragon::IWorker *evildragon::Workers<OclLaunchData>::create(Thread<OclLaunchData> *handle)
{
...
#ifdef evildragon_FEATURE_CUDA
template<>
evildragon::IWorker *evildragon::Workers<CudaLaunchData>::create(Thread<CudaLaunchData> *handle)
{
