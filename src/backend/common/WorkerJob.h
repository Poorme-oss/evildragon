...
#ifndef evildragon_WORKERJOB_H
#define evildragon_WORKERJOB_H
...
template<>
inline uint32_t *evildragon::WorkerJob<1>::nonce(size_t)
{
...
template<>
inline bool evildragon::WorkerJob<1>::nextRound(uint32_t rounds, uint32_t roundSize)
{
    bool ok = true;
    m_rounds[index()]++;
...
template<>
inline void evildragon::WorkerJob<1>::save(const Job &job, uint32_t reserveCount, Nonce::Backend backend)
{
    m_index           = job.index();
    m_jobs[index()]   = job;
    m_rounds[index()] = 0;
