...
cl_program evildragon::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\evildragon\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string evildragon::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string evildragon::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void evildragon::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
