...
void evildragon::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void evildragon::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void evildragon::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void evildragon::FindSharesKernel::setNonce(uint32_t nonce)
{
