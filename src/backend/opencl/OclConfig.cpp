...
#ifndef evildragon_OS_APPLE
evildragon::OclConfig::OclConfig() : m_platformVendor(kAMD) {}
#else
evildragon::OclConfig::OclConfig() = default;
#endif


evildragon::OclPlatform evildragon::OclConfig::platform() const
{
...
rapidjson::Value evildragon::OclConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<evildragon::OclLaunchData> evildragon::OclConfig::get(const Miner *miner, const Algorithm &algorithm, const OclPlatform &platform, const std::vector<OclDevice> &devices) const
{
...
void evildragon::OclConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void evildragon::OclConfig::generate()
{
    ...
    count += evildragon::generate<Algorithm::CN>(m_threads, devices);
    count += evildragon::generate<Algorithm::CN_LITE>(m_threads, devices);
    count += evildragon::generate<Algorithm::CN_HEAVY>(m_threads, devices);
    count += evildragon::generate<Algorithm::CN_PICO>(m_threads, devices);
    count += evildragon::generate<Algorithm::RANDOM_X>(m_threads, devices);
    count += evildragon::generate<Algorithm::ASTROBWT>(m_threads, devices);
    count += evildragon::generate<Algorithm::KAWPOW>(m_threads, devices);

    m_shouldSave = count > 0;
}


void evildragon::OclConfig::setDevicesHint(const char *devicesHint)
{
...
#ifndef evildragon_OS_APPLE
void evildragon::OclConfig::setPlatform(const rapidjson::Value &platform)
{
