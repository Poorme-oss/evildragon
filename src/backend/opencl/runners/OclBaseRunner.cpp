...
evildragon::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
#   ifdef evildragon_STRICT_OPENCL_CACHE
    m_deviceKey += ":";
...
evildragon::OclBaseRunner::~OclBaseRunner()
{
...
size_t evildragon::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t evildragon::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void evildragon::OclBaseRunner::build()
{
...
void evildragon::OclBaseRunner::init()
{
...
cl_mem evildragon::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t evildragon::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void evildragon::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void evildragon::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void evildragon::OclBaseRunner::finalize(uint32_t *hashOutput)
{
