...
cl_mem evildragon::OclSharedData::createBuffer(cl_context context, size_t size, size_t &offset, size_t limit)
{
...
uint64_t evildragon::OclSharedData::adjustDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
#   ifdef evildragon_INTERLEAVE_DEBUG
...
uint64_t evildragon::OclSharedData::resumeDelay(size_t id)
{
    if (m_threads < 2) {
        return 0;
    }
...
void evildragon::OclSharedData::release()
{
...
void evildragon::OclSharedData::setResumeCounter(uint32_t value)
{
    if (m_threads < 2) {
        return;
    }
...
void evildragon::OclSharedData::setRunTime(uint64_t time)
{
...
#ifdef evildragon_ALGO_RANDOMX
cl_mem evildragon::OclSharedData::dataset() const
{
    if (!m_dataset) {
...
void evildragon::OclSharedData::createDataset(cl_context ctx, const Job &job, bool host)
{
    if (m_dataset) {
        return;
    }
