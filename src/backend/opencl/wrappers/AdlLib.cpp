...
const char *evildragon::AdlLib::lastError() noexcept
{
    return uv_dlerror(&adlLib);
}


void evildragon::AdlLib::close()
{
    if (m_ready) {
...
AdlHealth evildragon::AdlLib::health(const OclDevice &device)
{
...
bool evildragon::AdlLib::dlopen()
{
    return uv_dlopen("atiadlxx.dll", &adlLib) == 0;
}


bool evildragon::AdlLib::load()
{
    try {
