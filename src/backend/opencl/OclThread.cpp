...
evildragon::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool evildragon::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value evildragon::OclThread::toJSON(rapidjson::Document &doc) const
{
