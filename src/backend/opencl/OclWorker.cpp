...
evildragon::OclWorker::OclWorker(size_t id, const OclLaunchData &data) :
...
evildragon::OclWorker::~OclWorker()
{
    delete m_runner;
}


void evildragon::OclWorker::jobEarlyNotification(const Job& job)
{
    if (m_runner) {
...
bool evildragon::OclWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t evildragon::OclWorker::intensity() const
{
    return m_runner ? m_runner->intensity() : 0;
}


void evildragon::OclWorker::start()
{
...
bool evildragon::OclWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::OPENCL) == 0) {
        return false;
    }
...
void evildragon::OclWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
