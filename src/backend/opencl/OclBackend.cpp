...
namespace evildragon { static const char *kAdlLabel = "ADL"; }
#endif
...
const char *evildragon::ocl_tag()
{
    return Tags::opencl();
}


evildragon::OclBackend::OclBackend(Controller *controller) :
...
evildragon::OclBackend::~OclBackend()
{
    delete d_ptr;
...
bool evildragon::OclBackend::isEnabled() const
{
    return d_ptr->controller->config()->cl().isEnabled() && OclLib::isInitialized() && d_ptr->platform.isValid() && !d_ptr->devices.empty();
}


bool evildragon::OclBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cl().threads().get(algorithm).isEmpty();
}


const evildragon::Hashrate *evildragon::OclBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const evildragon::String &evildragon::OclBackend::profileName() const
{
    return d_ptr->profileName;
}


const evildragon::String &evildragon::OclBackend::type() const
{
    return kType;
}


void evildragon::OclBackend::execCommand(char)
{
}


void evildragon::OclBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void evildragon::OclBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void evildragon::OclBackend::printHealth()
{
#   ifdef evildragon_FEATURE_ADL
    d_ptr->printHealth();
#   endif
...
void evildragon::OclBackend::setJob(const Job &job)
{
...
void evildragon::OclBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void evildragon::OclBackend::stop()
{
...
void evildragon::OclBackend::tick(uint64_t ticks)
{
...
#ifdef evildragon_FEATURE_API
rapidjson::Value evildragon::OclBackend::toJSON(rapidjson::Document &doc) const
{
...
void evildragon::OclBackend::handleRequest(IApiRequest &)
{
}
#endif
