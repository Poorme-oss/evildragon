...
evildragon::CpuLaunchData::CpuLaunchData(const Miner *miner, const Algorithm &algorithm, const CpuConfig &config, const CpuThread &thread) :
    algorithm(algorithm),
...
bool evildragon::CpuLaunchData::isEqual(const CpuLaunchData &other) const
{
...
evildragon::CnHash::AlgoVariant evildragon::CpuLaunchData::av() const
{
    if (intensity <= 2) {
...
const char *evildragon::CpuLaunchData::tag()
{
    return cpu_tag();
}
