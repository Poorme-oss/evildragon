...
const char *evildragon::backend_tag(uint32_t backend)
{
...
const char *evildragon::cpu_tag()
{
    return Tags::cpu();
}


evildragon::CpuBackend::CpuBackend(Controller *controller) :
...
evildragon::CpuBackend::~CpuBackend()
{
    delete d_ptr;
}


bool evildragon::CpuBackend::isEnabled() const
{
    return d_ptr->controller->config()->cpu().isEnabled();
}


bool evildragon::CpuBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cpu().threads().get(algorithm).isEmpty();
}


const evildragon::Hashrate *evildragon::CpuBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const evildragon::String &evildragon::CpuBackend::profileName() const
{
    return d_ptr->profileName;
}


const evildragon::String &evildragon::CpuBackend::type() const
{
    return kType;
}


void evildragon::CpuBackend::prepare(const Job &nextJob)
{
#   ifdef evildragon_ALGO_ARGON2
    const evildragon::Algorithm::Family f = nextJob.algorithm().family();
...
void evildragon::CpuBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void evildragon::CpuBackend::printHealth()
{
}


void evildragon::CpuBackend::setJob(const Job &job)
{
    if (!isEnabled()) {
        return stop();
    }
...
void evildragon::CpuBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void evildragon::CpuBackend::stop()
{
...
void evildragon::CpuBackend::tick(uint64_t ticks)
{
...
#ifdef evildragon_FEATURE_API
rapidjson::Value evildragon::CpuBackend::toJSON(rapidjson::Document &doc) const
{
...
void evildragon::CpuBackend::handleRequest(IApiRequest &request)
{
