...
evildragon::HwlocCpuInfo::HwlocCpuInfo()
{
    m_threads = 0;
...
bool evildragon::HwlocCpuInfo::membind(hwloc_const_bitmap_t nodeset)
{
    if (!hwloc_topology_get_support(m_topology)->membind->set_thisthread_membind) {
        return false;
    }
...
evildragon::CpuThreads evildragon::HwlocCpuInfo::threads(const Algorithm &algorithm, uint32_t limit) const
{
...
evildragon::CpuThreads evildragon::HwlocCpuInfo::allThreads(const Algorithm &algorithm, uint32_t limit) const
{
    CpuThreads threads;
    threads.reserve(m_threads);
...
void evildragon::HwlocCpuInfo::processTopLevelCache(hwloc_obj_t cache, const Algorithm &algorithm, CpuThreads &threads, size_t limit) const
{
#   ifndef evildragon_ARM
