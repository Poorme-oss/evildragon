...
bool evildragon::CpuConfig::isHwAES() const
{
    return (m_aes == AES_AUTO ? (Cpu::info()->hasAES() ? AES_HW : AES_SOFT) : m_aes) == AES_HW;
}


rapidjson::Value evildragon::CpuConfig::toJSON(rapidjson::Document &doc) const
{
...
size_t evildragon::CpuConfig::memPoolSize() const
{
    return m_memoryPool < 0 ? Cpu::info()->threads() : m_memoryPool;
}


std::vector<evildragon::CpuLaunchData> evildragon::CpuConfig::get(const Miner *miner, const Algorithm &algorithm) const
{
...
void evildragon::CpuConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void evildragon::CpuConfig::generate()
{
    ...
    count += evildragon::generate<Algorithm::CN>(m_threads, m_limit);
    count += evildragon::generate<Algorithm::CN_LITE>(m_threads, m_limit);
    count += evildragon::generate<Algorithm::CN_HEAVY>(m_threads, m_limit);
    count += evildragon::generate<Algorithm::CN_PICO>(m_threads, m_limit);
    count += evildragon::generate<Algorithm::RANDOM_X>(m_threads, m_limit);
    count += evildragon::generate<Algorithm::ARGON2>(m_threads, m_limit);
    count += evildragon::generate<Algorithm::ASTROBWT>(m_threads, m_limit);

    m_shouldSave |= count > 0;
}


void evildragon::CpuConfig::setAesMode(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
void evildragon::CpuConfig::setMemoryPool(const rapidjson::Value &value)
{
    if (value.IsBool()) {
