...
evildragon::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}


#ifdef evildragon_FEATURE_API
rapidjson::Value evildragon::NetworkState::getConnection(rapidjson::Document &doc, int version) const
{
...
rapidjson::Value evildragon::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void evildragon::NetworkState::printConnection() const
{
    if (!m_active) {
...
void evildragon::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *evildragon::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string evildragon::NetworkState::humanDiff(uint64_t diff)
{
...
void evildragon::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void evildragon::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void evildragon::NetworkState::onPause(IStrategy *strategy)
{
...
void evildragon::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t evildragon::NetworkState::latency() const
{
...
uint64_t evildragon::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t evildragon::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void evildragon::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
...
void evildragon::NetworkState::stop()
{
    m_active      = false;
    m_diff        = 0;
    m_ip          = nullptr;
