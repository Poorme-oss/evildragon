...
evildragon::EthStratumClient::EthStratumClient(int id, const char *agent, IClientListener *listener) :
    Client(id, agent, listener)
{
}


int64_t evildragon::EthStratumClient::submit(const JobResult& result)
{
...
void evildragon::EthStratumClient::login()
{
    m_results.clear();
...
void evildragon::EthStratumClient::onClose()
{
    m_authorized = false;
    Client::onClose();
}
...
bool evildragon::EthStratumClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void evildragon::EthStratumClient::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void evildragon::EthStratumClient::setExtraNonce(const rapidjson::Value &nonce)
{
    if (!nonce.IsString()) {
...
const char *evildragon::EthStratumClient::errorMessage(const rapidjson::Value &error) const
{
...
void evildragon::EthStratumClient::authorize()
{
    using namespace rapidjson;
...
void evildragon::EthStratumClient::onAuthorizeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    try {
        if (!success) {
...
void evildragon::EthStratumClient::onSubscribeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    if (!success) {
        return;
    }
...
void evildragon::EthStratumClient::subscribe()
{
    using namespace rapidjson;
