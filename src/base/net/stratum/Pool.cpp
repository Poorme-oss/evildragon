...
evildragon::Pool::Pool(const char *url) :
...
evildragon::Pool::Pool(const char *host, uint16_t port, const char *user, const char *password, int keepAlive, bool nicehash, bool tls, Mode mode) :
...
evildragon::Pool::Pool(const rapidjson::Value &object) :
    m_flags(1 << FLAG_ENABLED),
    m_pollInterval(kDefaultPollInterval),
...
bool evildragon::Pool::isEnabled() const
{
#   ifndef evildragon_FEATURE_TLS
    if (isTLS()) {
        return false;
    }
#   endif

#   ifndef evildragon_FEATURE_HTTP
...
bool evildragon::Pool::isEqual(const Pool &other) const
{
...
evildragon::IClient *evildragon::Pool::createClient(int id, IClientListener *listener) const
{
...
rapidjson::Value evildragon::Pool::toJSON(rapidjson::Document &doc) const
{
...
std::string evildragon::Pool::printableName() const
{
...
#ifdef APP_DEBUG
void evildragon::Pool::print() const
{
