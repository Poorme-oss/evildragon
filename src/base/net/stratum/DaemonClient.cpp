...
evildragon::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
evildragon::DaemonClient::~DaemonClient()
{
    delete m_timer;
}


bool evildragon::DaemonClient::disconnect()
{
...
bool evildragon::DaemonClient::isTLS() const
{
...
int64_t evildragon::DaemonClient::submit(const JobResult &result)
{
...
void evildragon::DaemonClient::connect()
{
...
void evildragon::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void evildragon::DaemonClient::onHttpData(const HttpData &data)
{
...
void evildragon::DaemonClient::onTimer(const Timer *)
{
...
bool evildragon::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash;
}


bool evildragon::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
bool evildragon::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t evildragon::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t evildragon::DaemonClient::rpcSend(const rapidjson::Document &doc)
{
...
void evildragon::DaemonClient::retry()
{
    m_failures++;
...
void evildragon::DaemonClient::send(const char *path)
{
...
void evildragon::DaemonClient::setState(SocketState state)
{
