...
evildragon::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener) :
    m_listener(listener)
{
...
evildragon::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


void evildragon::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void evildragon::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void evildragon::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool evildragon::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void evildragon::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void evildragon::SelfSelectClient::retry()
{
    setState(RetryState);
}


void evildragon::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void evildragon::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void evildragon::SelfSelectClient::onHttpData(const HttpData &data)
{
