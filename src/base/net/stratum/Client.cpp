...
evildragon::Client::Client(int id, const char *agent, IClientListener *listener) :
...
bool evildragon::Client::disconnect()
{
    m_keepAlive = 0;
    m_expire    = 0;
    m_failures  = -1;
...
bool evildragon::Client::isTLS() const
{
#   ifdef evildragon_FEATURE_TLS
...
const char *evildragon::Client::tlsFingerprint() const
{
#   ifdef evildragon_FEATURE_TLS
...
const char *evildragon::Client::tlsVersion() const
{
#   ifdef evildragon_FEATURE_TLS
    if (isTLS()) {
...
int64_t evildragon::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t evildragon::Client::send(const rapidjson::Value &obj)
{
...
int64_t evildragon::Client::submit(const JobResult &result)
{
#   ifndef evildragon_PROXY_PROJECT
...
void evildragon::Client::connect()
{
...
void evildragon::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void evildragon::Client::deleteLater()
{
    if (!m_listener) {
        return;
    }
...
void evildragon::Client::tick(uint64_t now)
{
...
void evildragon::Client::onResolved(const Dns &dns, int status)
{
...
bool evildragon::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
bool evildragon::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
#   endif
    ...
    if (m_jobs == 0) { // https://github.com/evildragon/evildragon/issues/459
        return false;
    }
...
bool evildragon::Client::send(BIO *bio)
{
#   ifdef evildragon_FEATURE_TLS
    uv_buf_t buf;
...
bool evildragon::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool evildragon::Client::write(const uv_buf_t &buf)
{
...
int evildragon::Client::resolve(const String &host)
{
...
int64_t evildragon::Client::send(size_t size)
{
...
void evildragon::Client::connect(sockaddr *addr)
{
...
void evildragon::Client::handshake()
{
    if (m_socks5) {
...
bool evildragon::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void evildragon::Client::onClose()
{
    delete m_socket;
...
void evildragon::Client::parse(char *line, size_t len)
{
    startTimeout();
...
void evildragon::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void evildragon::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void evildragon::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void evildragon::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void evildragon::Client::reconnect()
{
    if (!m_listener) {
        m_storage.remove(m_key);
...
void evildragon::Client::setState(SocketState state)
{
...
void evildragon::Client::startTimeout()
{
    m_expire = 0;
...
void evildragon::Client::onClose(uv_handle_t *handle)
{
...
void evildragon::Client::onConnect(uv_connect_t *req, int status)
{
...
void evildragon::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
