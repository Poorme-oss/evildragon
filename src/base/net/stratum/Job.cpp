...
evildragon::Job::Job(bool nicehash, const Algorithm &algorithm, const String &clientId) :
...
bool evildragon::Job::setSeedHash(const char *hash)
{
...
bool evildragon::Job::setTarget(const char *target)
{
    if (!target) {
        return false;
    }
...
void evildragon::Job::setDiff(uint64_t diff)
{
    m_diff   = diff;
...
void evildragon::Job::copy(const Job &other)
{
...
void evildragon::Job::move(Job &&other)
{
    m_algorithm  = other.m_algorithm;
    m_nicehash   = other.m_nicehash;
    m_size       = other.m_size;
