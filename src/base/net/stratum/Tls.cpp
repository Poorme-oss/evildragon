...
evildragon::Client::Tls::Tls(Client *client) :
    m_client(client)
{
...
bool evildragon::Client::Tls::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
bool evildragon::Client::Tls::send(const char *data, size_t size)
{
...
const char *evildragon::Client::Tls::fingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *evildragon::Client::Tls::version() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void evildragon::Client::Tls::read(const char *data, size_t size)
{
...
bool evildragon::Client::Tls::send()
{
    return m_client->send(m_write);
}


bool evildragon::Client::Tls::verify(X509 *cert)
{
    if (cert == nullptr) {
...
bool evildragon::Client::Tls::verifyFingerprint(X509 *cert)
{
