...
evildragon::BaseClient::BaseClient(int id, IClientListener *listener) :
    m_listener(listener),
    m_id(id)
{
}
...
void evildragon::BaseClient::setPool(const Pool &pool)
{
    if (!pool.isValid()) {
        return;
    }
...
bool evildragon::BaseClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == 1) {
        return false;
    }
...
bool evildragon::BaseClient::handleSubmitResponse(int64_t id, const char *error)
{
