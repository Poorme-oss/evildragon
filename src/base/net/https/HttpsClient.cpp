...
evildragon::HttpsClient::HttpsClient(FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
evildragon::HttpsClient::~HttpsClient()
{
    if (m_ctx) {
        SSL_CTX_free(m_ctx);
    }
...
const char *evildragon::HttpsClient::tlsFingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *evildragon::HttpsClient::tlsVersion() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void evildragon::HttpsClient::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
void evildragon::HttpsClient::read(const char *data, size_t size)
{
...
void evildragon::HttpsClient::write(std::string &&data, bool close)
{
...
bool evildragon::HttpsClient::verify(X509 *cert)
{
    if (cert == nullptr) {
        return false;
    }
...
bool evildragon::HttpsClient::verifyFingerprint(X509 *cert)
{
...
void evildragon::HttpsClient::flush(bool close)
{
