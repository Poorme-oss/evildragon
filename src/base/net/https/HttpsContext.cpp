...
evildragon::HttpsContext::HttpsContext(TlsContext *tls, const std::weak_ptr<IHttpListener> &listener) :
...
evildragon::HttpsContext::~HttpsContext() = default;


void evildragon::HttpsContext::append(char *data, size_t size)
{
    if (m_mode == TLS_AUTO) {
...
bool evildragon::HttpsContext::write(BIO *bio)
{
...
void evildragon::HttpsContext::parse(char *data, size_t size)
{
...
void evildragon::HttpsContext::shutdown()
{
    close();
}


void evildragon::HttpsContext::write(std::string &&data, bool close)
{
    m_close = close;
