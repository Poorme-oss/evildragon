...
evildragon::Http::Http() :
    m_host(kLocalhost)
{
}


bool evildragon::Http::isEqual(const Http &other) const
{
...
rapidjson::Value evildragon::Http::toJSON(rapidjson::Document &doc) const
{
...
void evildragon::Http::load(const rapidjson::Value &http)
{
    if (!http.IsObject()) {
        return;
    }
...
void evildragon::Http::setPort(int port)
{
