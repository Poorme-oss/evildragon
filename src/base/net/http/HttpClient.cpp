...
evildragon::HttpClient::HttpClient(FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
evildragon::HttpClient::~HttpClient()
{
    delete m_dns;
}


bool evildragon::HttpClient::connect()
{
    return m_dns->resolve(m_req.host);
}


void evildragon::HttpClient::onResolved(const Dns &dns, int status)
{
    this->status = status;
...
void evildragon::HttpClient::handshake()
{
...
void evildragon::HttpClient::read(const char *data, size_t size)
{
...
void evildragon::HttpClient::onConnect(uv_connect_t *req, int status)
{
