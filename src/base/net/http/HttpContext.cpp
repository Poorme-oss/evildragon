...
{
public:
    evildragon_DISABLE_COPY_MOVE_DEFAULT(HttpWriteBaton)
...
evildragon::HttpContext::HttpContext(int parser_type, const std::weak_ptr<IHttpListener> &listener) :
    HttpData(SEQUENCE++),
...
evildragon::HttpContext::~HttpContext()
{
    delete m_tcp;
    delete m_parser;
}
...
void evildragon::HttpContext::write(std::string &&data, bool close)
{
...
bool evildragon::HttpContext::isRequest() const
{
    return m_parser->type == HTTP_REQUEST;
}


size_t evildragon::HttpContext::parse(const char *data, size_t size)
{
    if (size == 0) {
        return size;
    }
...
std::string evildragon::HttpContext::ip() const
{
    char ip[46]           = {};
...
uint64_t evildragon::HttpContext::elapsed() const
{
    return Chrono::steadyMSecs() - m_timestamp;
}


void evildragon::HttpContext::close(int status)
{
...
evildragon::HttpContext *evildragon::HttpContext::get(uint64_t id)
{
...
void evildragon::HttpContext::closeAll()
{
    for (auto &kv : storage) {
...
int evildragon::HttpContext::onHeaderField(http_parser *parser, const char *at, size_t length)
{
...
int evildragon::HttpContext::onHeaderValue(http_parser *parser, const char *at, size_t length)
{
    auto ctx = static_cast<HttpContext*>(parser->data);
...
void evildragon::HttpContext::attach(http_parser_settings *settings)
{
...
void evildragon::HttpContext::setHeader()
{
