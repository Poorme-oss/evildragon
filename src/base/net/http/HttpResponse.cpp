...
evildragon::HttpResponse::HttpResponse(uint64_t id, int statusCode) :
    m_id(id),
...
bool evildragon::HttpResponse::isAlive() const
{
...
void evildragon::HttpResponse::end(const char *data, size_t size)
{
    if (!isAlive()) {
        return;
    }
