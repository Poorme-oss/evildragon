...
char *evildragon::NetBuffer::allocate()
{
    return getPool()->allocate();
}


void evildragon::NetBuffer::destroy()
{
    if (!pool) {
        return;
    }
...
void evildragon::NetBuffer::onAlloc(uv_handle_t *, size_t, uv_buf_t *buf)
{
...
void evildragon::NetBuffer::release(const char *buf)
{
    getPool()->deallocate(buf);
}


void evildragon::NetBuffer::release(const uv_buf_t *buf)
{
