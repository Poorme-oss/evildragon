...
static const evildragon::String kLocalHost("127.0.0.1");


evildragon::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
evildragon::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int evildragon::TcpServer::bind()
{
    if (!m_version) {
...
void evildragon::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void evildragon::TcpServer::onConnection(uv_stream_t *stream, int status)
{
