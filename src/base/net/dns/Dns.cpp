...
evildragon::Dns::Dns(IDnsListener *listener) :
    m_listener(listener)
{
...
bool evildragon::Dns::resolve(const String &host)
{
    if (m_host != host) {
        m_host = host;
...
const char *evildragon::Dns::error() const
{
    return uv_strerror(m_status);
}


const evildragon::DnsRecord &evildragon::Dns::get(DnsRecord::Type prefered) const
{
    if (count() == 0) {
...
size_t evildragon::Dns::count(DnsRecord::Type type) const
{
    if (type == DnsRecord::A) {
        return m_ipv4.size();
    }
...
void evildragon::Dns::onResolved(int status, addrinfo *res)
{
    m_status = status;
...
void evildragon::Dns::onResolved(uv_getaddrinfo_t *req, int status, addrinfo *res)
{
