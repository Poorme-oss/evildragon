...
evildragon::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


evildragon::TlsContext *evildragon::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool evildragon::TlsContext::load(const TlsConfig &config)
{
...
bool evildragon::TlsContext::setCiphers(const char *ciphers)
{
...
bool evildragon::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool evildragon::TlsContext::setDH(const char *dhparam)
{
    DH *dh = nullptr;
...
void evildragon::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
