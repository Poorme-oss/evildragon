...
evildragon::ServerTls::ServerTls(SSL_CTX *ctx) :
    m_ctx(ctx)
{
}


evildragon::ServerTls::~ServerTls()
{
    if (m_ssl) {
        SSL_free(m_ssl);
    }
...
bool evildragon::ServerTls::isTLS(const char *data, size_t size)
{
...
bool evildragon::ServerTls::send(const char *data, size_t size)
{
...
void evildragon::ServerTls::read(const char *data, size_t size)
{
    if (!m_ssl) {
        m_ssl = SSL_new(m_ctx);
...
void evildragon::ServerTls::read()
{
