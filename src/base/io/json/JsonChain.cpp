...
evildragon::JsonChain::JsonChain() = default;


bool evildragon::JsonChain::add(rapidjson::Document &&doc)
{
...
bool evildragon::JsonChain::addFile(const char *fileName)
{
...
bool evildragon::JsonChain::addRaw(const char *json)
{
    using namespace rapidjson;
    Document doc;
...
void evildragon::JsonChain::dump(const char *fileName)
{
...
bool evildragon::JsonChain::getBool(const char *key, bool defaultValue) const
{
...
const char *evildragon::JsonChain::getString(const char *key, const char *defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &evildragon::JsonChain::getArray(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &evildragon::JsonChain::getObject(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &evildragon::JsonChain::getValue(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
        if (i != it->MemberEnd()) {
...
int evildragon::JsonChain::getInt(const char *key, int defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
int64_t evildragon::JsonChain::getInt64(const char *key, int64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
uint64_t evildragon::JsonChain::getUint64(const char *key, uint64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
unsigned evildragon::JsonChain::getUint(const char *key, unsigned defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
