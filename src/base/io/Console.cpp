...
evildragon::Console::Console(IConsoleListener *listener)
    : m_listener(listener)
{
    if (!isSupported()) {
        return;
...
evildragon::Console::~Console()
{
    stop();
}


void evildragon::Console::stop()
{
    if (!m_tty) {
        return;
    }
...
bool evildragon::Console::isSupported() const
{
...
void evildragon::Console::onAllocBuffer(uv_handle_t *handle, size_t, uv_buf_t *buf)
{
...
void evildragon::Console::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
    if (nread < 0) {
