...
{
public:
    evildragon_DISABLE_COPY_MOVE(LogPrivate)
...
void evildragon::Log::add(ILogBackend *backend)
{
    if (d) {
        d->add(backend);
    }
...
void evildragon::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void evildragon::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
