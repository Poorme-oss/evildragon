...
bool evildragon::FileLogWriter::open(const char *fileName)
{
...
bool evildragon::FileLogWriter::write(const char *data, size_t size)
{
    if (!isOpen()) {
        return false;
    }
...
bool evildragon::FileLogWriter::writeLine(const char *data, size_t size)
{
    uv_buf_t buf[2] = {
