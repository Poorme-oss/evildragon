...
evildragon::Arguments::Arguments(int argc, char **argv) :
    m_argv(argv),
    m_argc(argc)
{
...
bool evildragon::Arguments::hasArg(const char *name) const
{
    if (m_argc == 1) {
        return false;
    }
...
const char *evildragon::Arguments::value(const char *key1, const char *key2) const
{
...
void evildragon::Arguments::add(const char *arg)
{
    if (arg == nullptr) {
        return;
    }
