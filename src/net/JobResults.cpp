...
#if defined(evildragon_FEATURE_OPENCL) || defined(evildragon_FEATURE_CUDA)
...
{
public:
    evildragon_DISABLE_COPY_MOVE_DEFAULT(JobResultsPrivate)
...
void evildragon::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void evildragon::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void evildragon::JobResults::stop()
{
    assert(handler != nullptr);
...
void evildragon::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void evildragon::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(evildragon_FEATURE_OPENCL) || defined(evildragon_FEATURE_CUDA)
void evildragon::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
