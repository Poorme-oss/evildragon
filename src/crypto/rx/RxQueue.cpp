...
evildragon::RxQueue::RxQueue(IRxListener *listener) :
    m_listener(listener)
{
...
evildragon::RxDataset *evildragon::RxQueue::dataset(const Job &job, uint32_t nodeId)
{
...
evildragon::HugePagesInfo evildragon::RxQueue::hugePages()
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
template<typename T>
bool evildragon::RxQueue::isReady(const T &seed)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
void evildragon::RxQueue::enqueue(const RxSeed &seed, const std::vector<uint32_t> &nodeset, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
template<typename T>
bool evildragon::RxQueue::isReadyUnsafe(const T &seed) const
{
    return m_storage != nullptr && m_storage->isAllocated() && m_state == STATE_IDLE && m_seed == seed;
}


void evildragon::RxQueue::backgroundInit()
{
...
void evildragon::RxQueue::onReady()
{
    std::unique_lock<std::mutex> lock(m_mutex);
