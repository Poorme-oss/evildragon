...
evildragon::RxCache::RxCache(bool hugePages, uint32_t nodeId)
{
...
evildragon::RxCache::RxCache(uint8_t *memory)
{
    create(memory);
}


evildragon::RxCache::~RxCache()
{
...
bool evildragon::RxCache::init(const Buffer &seed)
{
    if (m_seed == seed) {
        return false;
    }
...
evildragon::HugePagesInfo evildragon::RxCache::hugePages() const
{
    return m_memory ? m_memory->hugePages() : HugePagesInfo();
}


void evildragon::RxCache::create(uint8_t *memory)
{
    if (!memory) {
        return;
    }
