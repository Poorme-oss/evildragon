...
bool evildragon::RxConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
rapidjson::Value evildragon::RxConfig::toJSON(rapidjson::Document &doc) const
{
...
#ifdef evildragon_FEATURE_HWLOC
std::vector<uint32_t> evildragon::RxConfig::nodeset() const
{
    if (!m_nodeset.empty()) {
        return m_nodeset;
    }
...
const char *evildragon::RxConfig::modeName() const
{
    return modeNames[m_mode];
}


uint32_t evildragon::RxConfig::threads(uint32_t limit) const
{
    if (m_threads > 0) {
        return m_threads;
    }
...
#ifdef evildragon_FEATURE_MSR
const char *evildragon::RxConfig::msrPresetName() const
{
    return modNames[msrMod()];
}


const evildragon::MsrItems &evildragon::RxConfig::msrPreset() const
{
...
uint32_t evildragon::RxConfig::msrMod() const
{
    if (!wrmsr()) {
...
void evildragon::RxConfig::readMSR(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
evildragon::RxConfig::Mode evildragon::RxConfig::readMode(const rapidjson::Value &value) const
{
    if (value.IsUint()) {
