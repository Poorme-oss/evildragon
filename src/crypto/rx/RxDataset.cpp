...
evildragon::RxDataset::RxDataset(bool hugePages, bool oneGbPages, bool cache, RxConfig::Mode mode, uint32_t node) :
    m_mode(mode),
    m_node(node)
{
...
evildragon::RxDataset::RxDataset(RxCache *cache) :
    m_node(0),
    m_cache(cache)
{
}
...
evildragon::RxDataset::~RxDataset()
{
...
bool evildragon::RxDataset::init(const Buffer &seed, uint32_t numThreads, int priority)
{
...
bool evildragon::RxDataset::isHugePages() const
{
    return m_memory && m_memory->isHugePages();
}


bool evildragon::RxDataset::isOneGbPages() const
{
    return m_memory && m_memory->isOneGbPages();
}


evildragon::HugePagesInfo evildragon::RxDataset::hugePages(bool cache) const
{
...
size_t evildragon::RxDataset::size(bool cache) const
{
    size_t size = 0;
...
void *evildragon::RxDataset::raw() const
{
    return m_dataset ? randomx_get_dataset_memory(m_dataset) : nullptr;
}


void evildragon::RxDataset::setRaw(const void *raw)
{
    if (!m_dataset) {
        return;
    }
...
void evildragon::RxDataset::allocate(bool hugePages, bool oneGbPages)
{
...
uint8_t* evildragon::RxDataset::tryAllocateScrathpad()
{
