...
{
public:
    evildragon_DISABLE_COPY_MOVE_DEFAULT(RxNUMAStoragePrivate)
...
evildragon::RxNUMAStorage::RxNUMAStorage(const std::vector<uint32_t> &nodeset) :
    d_ptr(new RxNUMAStoragePrivate(nodeset))
{
}


evildragon::RxNUMAStorage::~RxNUMAStorage()
{
    delete d_ptr;
}


bool evildragon::RxNUMAStorage::isAllocated() const
{
    return d_ptr->isAllocated();
}


evildragon::HugePagesInfo evildragon::RxNUMAStorage::hugePages() const
{
...
evildragon::RxDataset *evildragon::RxNUMAStorage::dataset(const Job &job, uint32_t nodeId) const
{
...
void evildragon::RxNUMAStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode, int priority)
{
    d_ptr->setSeed(seed);
