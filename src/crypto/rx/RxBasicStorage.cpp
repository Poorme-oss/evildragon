...
{
public:
    evildragon_DISABLE_COPY_MOVE(RxBasicStoragePrivate)
...
evildragon::RxBasicStorage::RxBasicStorage() :
...
bool evildragon::RxBasicStorage::isAllocated() const
{
    return d_ptr->dataset() && d_ptr->dataset()->cache() && d_ptr->dataset()->cache()->get();
}


evildragon::HugePagesInfo evildragon::RxBasicStorage::hugePages() const
{
    if (!d_ptr->dataset()) {
        return {};
    }
...
evildragon::RxDataset *evildragon::RxBasicStorage::dataset(const Job &job, uint32_t) const
{
...
void evildragon::RxBasicStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    d_ptr->setSeed(seed);
