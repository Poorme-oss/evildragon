...
#if defined(evildragon_OS_LINUX)
#   if (defined(MAP_HUGE_1GB) || defined(MAP_HUGE_SHIFT))
#       define evildragon_HAS_1GB_PAGES
#   endif
#   include "crypto/common/LinuxMemory.h"
#endif


bool evildragon::VirtualMemory::isHugepagesAvailable()
{
    return true;
}


bool evildragon::VirtualMemory::isOneGbPagesAvailable()
{
#   ifdef evildragon_HAS_1GB_PAGES
...
void *evildragon::VirtualMemory::allocateExecutableMemory(size_t size)
{
#   if defined(__APPLE__)
...
void *evildragon::VirtualMemory::allocateLargePagesMemory(size_t size)
{
#   if defined(__APPLE__)
...
void *evildragon::VirtualMemory::allocateOneGbPagesMemory(size_t size)
{
#   ifdef evildragon_HAS_1GB_PAGES
...
void evildragon::VirtualMemory::flushInstructionCache(void *p, size_t size)
{
...
void evildragon::VirtualMemory::freeLargePagesMemory(void *p, size_t size)
{
    munmap(p, size);
}


void evildragon::VirtualMemory::protectExecutableMemory(void *p, size_t size)
{
    mprotect(p, size, PROT_READ | PROT_EXEC);
}


void evildragon::VirtualMemory::unprotectExecutableMemory(void *p, size_t size)
{
    mprotect(p, size, PROT_WRITE | PROT_EXEC);
}


void evildragon::VirtualMemory::osInit(bool)
{
}


bool evildragon::VirtualMemory::allocateLargePagesMemory()
{
#   if defined(evildragon_OS_LINUX)
...
bool evildragon::VirtualMemory::allocateOneGbPagesMemory()
{
#   if defined(evildragon_HAS_1GB_PAGES)
...
void evildragon::VirtualMemory::freeLargePagesMemory()
{
