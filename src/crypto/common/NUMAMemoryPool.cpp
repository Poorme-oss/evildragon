...
evildragon::NUMAMemoryPool::NUMAMemoryPool(size_t size, bool hugePages) :
...
evildragon::NUMAMemoryPool::~NUMAMemoryPool()
{
    for (auto kv : m_map) {
        delete kv.second;
    }
...
bool evildragon::NUMAMemoryPool::isHugePages(uint32_t node) const
{
    if (!m_size) {
        return false;
    }
...
uint8_t *evildragon::NUMAMemoryPool::get(size_t size, uint32_t node)
{
    if (!m_size) {
        return nullptr;
    }
...
void evildragon::NUMAMemoryPool::release(uint32_t node)
{
...
evildragon::IMemoryPool *evildragon::NUMAMemoryPool::get(uint32_t node) const
{
    return m_map.count(node) ? m_map.at(node) : nullptr;
}


evildragon::IMemoryPool *evildragon::NUMAMemoryPool::getOrCreate(uint32_t node) const
{
    auto pool = get(node);
    if (!pool) {
